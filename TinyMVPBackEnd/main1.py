...
def main():
    ...
    parser = PhraseParser()
    phrases = parser.parse(raw_text)  # list of {"en": ..., "es": ...}
    ...
    tts = TTSEngine(model=config.MODEL, voice=config.VOICE)
    ...
    for idx, p in enumerate(phrases, start=1):
        ...
        tts.synthesize(text_en, str(out_file))
        normal_files.append(str(out_file))

    # 5. Create full lesson (normal)
    full_normal = lesson_root / "full_normal.mp3"
    print(f"\nCombining into {full_normal.name} with {config.SILENCE_BETWEEN_PHRASES_MS} ms pauses...")
    post.add_silence_between(
        normal_files,
        full_normal,
        silence_ms=config.SILENCE_BETWEEN_PHRASES_MS
    )
